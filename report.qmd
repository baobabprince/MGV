...
# map |>  filter(sample_ID |>  str_detect("h", negate = T)) |> filter(pnid != "MGV11") |> 
...
exp@otu_table |> data.frame() |> 
  ...
  left_join(ord |> cbind(clustered) |> rownames_to_column("name")) |> 
  group_by(clustered) |> 
  summarise(ASV = ASV, Taxon = Taxon, RA = value/sum(value)) |> 
  arrange(desc(RA)) |> slice_head(n = 5) |> ungroup() |> 
  gt::gt(rowname_col = F) |> fmt_percent(columns = RA, decimals = 2)
```
